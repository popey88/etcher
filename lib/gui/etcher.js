...
const WINDOW_CLOSE_TIMEOUT_MS = 500
let mainWindow = null
...
electron.app.on('ready', () => {
  ...
  // and can cause a disconnected window (displaying blank contents) to remain,
  // we attempt to force-close & quit after a timeout to ensure that the window
  // will complete it's closing.
  // See https://github.com/electron/electron/issues/5273
  mainWindow.on('close', () => {
    debug('window:close')
    clearTimeout(windowTimeout)
    windowTimeout = setTimeout(() => {
      debug('window:close:timeout')
      mainWindow.close()
      electron.app.quit()
